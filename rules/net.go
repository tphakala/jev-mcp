...
// than superseded by it: nosprintfhostport only matches a scheme-prefixed URL
// ("scheme://%s:..."), never the bare "%s:%d" / "%v:%d" form this rule targets,
// which is the common case for building a net.Dial, net.Listen or
...
func FilepathIsLocal(m dsl.Matcher) {
	...
	// ".." substring test (version ranges, ellipsis checks, arbitrary text),
	// and there is no type signal that separates a path from any other string.
	// So the rule keys on the operand's name instead and stays advisory.
	//
	// Two guards keep it narrow:
	//
	//  1. The operand must be a bare identifier. The name match runs on the
	//     operand's full source text, so without this a selector such as
	//     req.URL.Path would match on "Path" even though URL paths are the
	//     documented exception.
	//  2. The identifier must read like a path, keyed on camelCase boundaries.
	//     Both alternatives are anchored (| binds loosest, so an unanchored
	//     branch would match the keyword inside any identifier, e.g. MyDirtyVar
	//     via "Dir"): a lowercase keyword that is the whole name, a plural, a
	//     common compound (filepath, filename, dirname), or the first camelCase
	//     component (fileName, dirEntry); or a capitalized keyword at a camelCase
	//     boundary (userPath, srcDir, inputFile).
	//
	// For URL paths strings.Contains stays appropriate: filepath.IsLocal cleans
	// the path first, so "path/../etc" becomes "etc" and passes.
	m.Match(
...
func DeprecatedReverseProxyDirector(m dsl.Matcher) {
	...
	// Both the pointer and the value type: ReverseProxy is normally used as
	// *ReverseProxy, but a value embedded in another struct or held in a local
	// variable assigns through the same syntax.
	m.Match(
		`$proxy.Director = $_`,
	).
...
func ErrorBeforeUse(m dsl.Matcher) {
	...
	// Four statement shapes are covered for each constructor:
	//   - single-value assignment: name := f.Method(...)
	//   - bare call: f.Method(...)
	//   - multi-value assignment: a, b := f.Method(...)
	//   - deferred call: defer f.Close() before the err check, which runs on a
	//     nil receiver and panics when the open failed
	m.Match(
