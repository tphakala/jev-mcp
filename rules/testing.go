...
func BenchmarkLoop(m dsl.Matcher) {
	...
	// Pattern 3: for range b.N (no variable). Auto-fixable: there is no loop
	// variable to drop, so b.Loop() is a direct substitution. The Suggest names
	// the body capture as $body (no star); the $*body spelling belongs to the
	// match pattern and would be written out literally. MEASURED against
	// golangci-lint 2.13.2: --fix rewrites multi-statement and empty bodies
	// into compiling b.Loop() loops.
	m.Match(
		`for range $b.N { $*body }`,
	).
		Where(m["b"].Type.Is("*testing.B")).
...
// and suggests using t.Context() instead. The matcher is gated on the _test.go
// file name, so it fires anywhere in a test file, including test helpers, not
...
//
// Report-only, no autofix: the rewrite drops the only time.Sleep call, so when
// $d does not itself mention time and the file has no other time.* use, --fix
// would orphan the "time" import (which it does not prune) and the file would
// no longer compile.
//
