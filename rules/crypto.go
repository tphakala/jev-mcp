...
func WeakRSAKeySize(m dsl.Matcher) {
	// One constant predicate covers every weak size (512, 768, 1024, 1536, ...)
	// instead of enumerating literals; sizes under 1024 are also rejected
	// outright by Go 1.24+. A non-constant bits argument is left alone.
	m.Match(
...
//	cfg := &tls.Config{}
//
//	// Deterministic tests (testing/cryptotest, Go 1.26+):
