...
- Provider error messages keep TypeSafe's `error_type` when it is short (for example `api_usage_error: Unknown model`). Provider-supplied text in errors and in `doctor`'s output (messages, request ids, and the probed model and id), and the text of transport errors, has control characters, Unicode bidirectional controls and line separators, and invalid UTF-8 replaced, and is capped at 512 bytes.
