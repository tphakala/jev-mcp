...
- `TYPESAFE_API_KEY` and `OPENROUTER_API_KEY` are trimmed the same way. A key that is only whitespace, or that holds an ASCII control character other than tab, stops the server at startup with an error naming the variable whenever that provider would be used, including as the fallback. Before, such a key failed each call (a 401 or a retried transport error) and fell back to the other provider when there was one.
